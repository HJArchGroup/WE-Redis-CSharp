...
namespace ServiceStack.Cluster
{
    public class CRC16
    {
        public static ushort Compute(string s)
        {
            ushort result = 0x0000;
            int len = s.Length;
            for (int i = 0; i < len; ++i)
            {
                result = (ushort)((result << 8) ^ CRC16.LOOKUP_TABLE[((result >> 8) ^ s[i]) & 0x00FF]);
            }
            return result;
        }

        private static ushort[] LOOKUP_TABLE = { 0x0000, 0x1021, 0x2042, 0x3063,
